...
# URLs to the Hashcat package and its dependencies
packages = [
    "http://http.us.debian.org/debian/pool/main/h/hashcat/hashcat_6.2.6+ds2-1+b1_arm64.deb",
    "http://http.us.debian.org/debian/pool/main/m/minizip/libminizip1_1.1-8+b1_arm64.deb"  # libminizip dependency
]
package_names = [url.split("/")[-1] for url in packages]
...
with tempfile.TemporaryDirectory() as extract_dir:
    ...
    # Download and process each package
    for url, package_name in zip(packages, package_names):
    ...
    # Verify that Hashcat recognizes the new library
    result = subprocess.run(["ldd", "usr/bin/hashcat"], capture_output=True, text=True)
    if "libminizip.so.1 => not found" in result.stdout:
        print("Error: libminizip.so.1 is still not found.")
    else:
        print("libminizip.so.1 is recognized successfully.")

print("Hashcat and its dependencies installed successfully!")
